...
void low_level_do(uint32_t unused, uint32_t status)
{
...
void high_level_do(uint32_t unused, uint32_t status)
{
	if (status == THRESHOLD_ENABLE_LCR) {
		if (get_led_status(LED_IDX_E_COOLANT) != LED_START_BLINK)
			led_ctrl(LED_IDX_E_COOLANT, LED_START_BLINK);
...
void high_coolant_temp_do(uint32_t unused, uint32_t status)
{
	if (status == THRESHOLD_STATUS_UCR)
		deassert_all_rpu_ready_pin();
}

void flow_trigger_do(uint32_t unused, uint32_t status)
{
	if (status == THRESHOLD_STATUS_LCR)
		deassert_all_rpu_ready_pin();
}
...
sensor_threshold threshold_tbl[] = {
	...
	{ SENSOR_NUM_FAN_PRSNT, THRESHOLD_ENABLE_DISCRETE, 0, 0, fb_prsnt_handle, THRESHOLD_ARG0_TABLE_INDEX, 0}
};

#define SENSOR_NUM(type, index) SENSOR_NUM_FB_##index##_##type
#define FB_SEN_ENTRY(index) \
    {{ SENSOR_NUM(FAN_TACH_RPM, index), SENSOR_NUM(HSC_TEMP_C, index), SENSOR_NUM(HEX_OUTLET_TEMP_C, index) }}
...
static uint8_t fb_reinit(uint8_t idx)
{
	...
	for (uint8_t i = 0; i < FB_REINIT_SEN_COUNT; i++) {
		if (commom_tbl_sen_reinit(fb_sen_tbl[idx].sen_num[i]))
	...
	while (1) {
		...
		for (uint8_t i = 0; i < ARRAY_SIZE(threshold_tbl); i++) {
			...
			if (threshold_tbl[i].type == THRESHOLD_ENABLE_DISCRETE) {
				...
				int reading = 0;
				uint8_t status = get_sensor_reading(sensor_config, sensor_config_count, threshold_tbl[i].sensor_num,
					&reading, GET_FROM_CACHE);

				if (status != SENSOR_READ_4BYTE_ACUR_SUCCESS) {
					LOG_ERR("0x%02x get sensor cache fail", threshold_tbl[i].sensor_num);
					continue;
				}

				LOG_ERR("threshold_tbl[i].last_value = %x, reading = %x", threshold_tbl[i].last_value, reading);
