...
static uint16_t get_fb_pwr_prsnt(void)
{
	...
	uint8_t input_reg = 0;
	I2C_MSG msg =
		construct_i2c_message(I2C_BUS9, FB_PWR_PRSNT_ADDR_EVEN >> 1, 1, &input_reg, 1); // read even io expander
	...
	msg = construct_i2c_message(I2C_BUS9, FB_PWR_PRSNT_ADDR_ODD >> 1, 1, &input_reg, 1); // read odd io expander
	if (i2c_master_read(&msg, i2c_retry)) {
...
static uint16_t get_fb_sig_prsnt(void)
{
	...
	uint8_t input_reg = 0;
	I2C_MSG msg =
		construct_i2c_message(I2C_BUS4, FB_SIG_PRSNT_ADDR_EVEN >> 1, 1, &input_reg, 1); // read even io expander
	if (i2c_master_read(&msg, i2c_retry)) {
	...
	msg = construct_i2c_message(I2C_BUS4, FB_SIG_PRSNT_ADDR_ODD >> 1, 1, &input_reg, 1); // read odd io expander
	if (i2c_master_read(&msg, i2c_retry)) {
...
bool get_fb_present_status(uint16_t *fb_present_status)
{
	...
	*fb_present_status = pwr_prsnt & sig_prsnt;
	LOG_INF("pwr_prsnt = %x, sig_prsnt = %x, fb_present_status = %x", pwr_prsnt, sig_prsnt, *fb_present_status);
...
{
	// TODO: need modify fan index for new design
	return false;
	...
	if (sen_num == SENSOR_NUM_FB_1_FAN_TACH_RPM) {
		get_fb_present_status(&fb_present_status);
